...
exports.copyProject= function(req, res) {
    ...
    for (i=1;i<numberOfTasks;i++) {
        ...
        //create date (cd) is used as the unique id for updates so need to make sure they are slightly different (add seconds)
        additionalTask.cd = new Date();
        additionalTask.cd.setSeconds(additionalTask.cd.getSeconds() + i);
        newTasks.push(additionalTask);
    }

    // retrieve latest version of Project and add the new Tasks to it
