...
var fs = require('fs');
var thumbnail = require('../controllers/ogcThumbnail');
...
var officeExtensions = ['ppt', 'doc', 'xls', 'pdf', 'pptx','docx','xlxs'];
    ...
    db.collection('ogcFiles', function(err, collection) {
        collection.insert(object, {safe: true}, function(err, result) {
            if (err) {
                ...
                //go and make a thumbnail
                var thumbnailUrl = thumbnail.generate(result[0]);
                console.log('thumbnail url : ' + JSON.stringify(thumbnailUrl));
                var response = _.assign(result[0],thumbnailUrl);
...
exports.uploadFiles = function(req, res) {
    ...
    fileUploadResponse.targetPath = target_path.substring(5);
    ...
    console.log('file upload : ' + JSON.stringify(fileUploadResponse));
    // move the file from the temporary location to the intended location
    fs.rename(tmp_path, target_path, function(err) {
        if (err) throw err;
        // delete the temporary file, so that the explicitly set temporary upload dir does not get filled with unwanted files
